...
quantile of two-period demand~\cite{zipkin},
\begin{equation}
...
seasonality); and (iv) an $(s,S)$ variant~\cite{zhengfedergruen} for the fixed-order-cost case. To compete on the same
...
base-stock. The update is the standard Q-learning rule~\cite{suttonbarto}:
\begin{equation}
...
A Stable-Baselines3~\cite{sb3} PPO~\cite{ppo} agent trains on a Gymnasium~\cite{gymnasium} wrapper
of the same environment; PPO is the on-policy workhorse of modern deep reinforcement learning, the
family launched by value-based DQN~\cite{mnih}. It is the
